...
namespace numtheo_n {
	...
	template<i32 P> MIP<P> qpow_signed(MIP<P>, i64);
	template<i32 P> MIP<P> inv(MIP<P>);
	// discrete log
	...
	public:
		...
		using Base::set_mod;
		// multiplicative inverse
		friend MIP<P> qpow_signed<>(MIP<P> x, i64 y);
		static void lin_inv_preproc(u32 N);
		static void O1inv_preproc();
		friend MIP<P> inv<>(MIP<P> x);
		MIP<P> operator/(MIP<P> x) const {
			return *this * inv(x);
		}
		MIP<P> &operator/=(MIP<P> x) {
		...
		// quadradic residue
		friend i32 legendre<>(MIP<P>);
		friend std::optional<MIP<P>> sqrt<>(MIP<P>);
	};
